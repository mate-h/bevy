...
use bevy_render::{
    ...
    render_resource::{
        ...
        BindGroupLayoutEntries, Buffer, BufferBinding, BufferInitDescriptor, BufferUsages,
        BindingResource, CachedComputePipelineId, ComputePassDescriptor, ComputePipelineDescriptor,
...
pub fn prepare_generated_environment_map_bind_groups(
    ...
    for (entity, textures, env_map_light) in &light_probes {
        ...
        for dst_mip in 1..mip_count {
            ...
            let src_view = textures.environment_map.texture.create_view(&TextureViewDescriptor {
        ...
        let stbn_texture_view = stbn_texture.texture.clone().create_view(&TextureViewDescriptor {
            dimension: Some(TextureViewDimension::D2Array),
            ..Default::default()
        });
        ...
        let irradiance_map = env_map_light.diffuse_map.texture.create_view(&TextureViewDescriptor {
            dimension: Some(TextureViewDimension::D2Array),
            ..Default::default()
        });
        ...
        let src_view = env_map_light.environment_map.texture.create_view(&TextureViewDescriptor {
            dimension: Some(TextureViewDimension::D2Array),
            ..Default::default()
        });
