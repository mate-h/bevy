    ...
    F: 'static + Send + Sync,
{
    fn build(&self, app: &mut App) {
        app.add_plugins(
            bevy_render::sync_component::SyncComponentPlugin::<C, RenderApp, F>::default(),
        );
...
where
    EI: ExtractInstance<RenderApp> + Clone,
{
    ...
    EI: ExtractInstance<RenderApp> + Clone,
{
    fn build(&self, app: &mut App) {
        ...
        render_app.init_resource::<ExtractedInstances<EI, RenderApp>>();
        render_app.add_systems(
            ExtractSchedule,
...
pub fn extract_instances_from_cameras<EI>(
    mut extracted_instances: ResMut<ExtractedInstances<EI, RenderApp>>,
