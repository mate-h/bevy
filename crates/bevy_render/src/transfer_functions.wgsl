...
// peak, γ = 1.2 at 1000 nits per BT.2100-2 eq. 5). Feeding display-referred
// tone-mapped output through this OETF double-tone-maps; a correct HLG path
// needs a scene-referred encoder input, which Bevy does not have.

const HLG_A: f32 = 0.17883277;
const HLG_B: f32 = 0.28466892; // 1 - 4 * HLG_A
const HLG_C: f32 = 0.55991073; // 0.5 - HLG_A * ln(4 * HLG_A)

// HLG OETF for one channel: scene-linear E in [0, 1] → signal in [0, 1].
//
//   E' = sqrt(3 * E)               if E <= 1/12
//   E' = a * ln(12 * E - b) + c    otherwise
//
// Negative inputs are clamped to 0 (sqrt of a negative is indeterminate).
fn hlg_oetf_channel(e_in: f32) -> f32 {
    let e = max(e_in, 0.0);
    if e <= 1.0 / 12.0 {
        return sqrt(3.0 * e);
    }
...
// Per-channel HLG OETF; see `hlg_oetf_channel`.
fn hlg_oetf(e: vec3<f32>) -> vec3<f32> {
    return vec3(
        hlg_oetf_channel(e.x),
        hlg_oetf_channel(e.y),
        hlg_oetf_channel(e.z),
    );
}
